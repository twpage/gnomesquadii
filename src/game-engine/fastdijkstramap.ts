...
let flood_fill_fn = (x: number, y: number, level: Bones.Region, pathable_fn: IPathableFunction, local_map: IDijkstraMap, passable: IStorageMap, blocked: IStorageMap) => {
    ...
    let key = Bones.Utils.x_and_yToKey(x, y).toString()
    ...
    let terrain_at = level.terrain.entities[key]
...
export interface IPathableFunction {
    (terrain_at: Bones.Entities.Terrain) : boolean
}

export let PathFunctionWalkable : IPathableFunction
PathFunctionWalkable = function(terrain_at: Bones.Entities.Terrain): boolean {
    return (!(terrain_at.blocksWalking))
}

export let PathFunctionFlyable : IPathableFunction
PathFunctionFlyable = function(terrain_at: Bones.Entities.Terrain): boolean {
    return (!(terrain_at.blocksFlying))
}

export function setupDijkstraMap(level: Bones.Region, start_xy: Bones.Coordinate, pathable_fn: IPathableFunction) : IDijkstraMap {
